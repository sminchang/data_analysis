...
if __name__ == "__main__":
    input_path = '국가재정_2021_2023.xlsx'
    output_dir = r'c:\Users\User\OneDrive - 씨지인사이드\pc1_downloads'

    if not os.path.exists(output_dir, exist_ok=True):
            os.makedirs(output_dir)
    df = pd.read_excel(input_path, engine='openpyxl')  # 읽어올 Excel 경로
    download_file(df, output_dir)  # 파일 다운로드
