...
def pdf_table_extract(input_path, output_file):
    ...
    for file_name in os.listdir(input_path):
        ...
        # 한 파일에 한 문건만 있다고 가정하며, 한 파일 내 여러 문건이 있는 경우는 예외처리
        document_set = {
            # A 테이블
        ...
        if file_name.endswith('.pdf'):
            ...
            try:
                ...
                with pdfplumber.open(file_path) as pdf:
                    ...
                    # 파일이 다중 문건 정보를 담고 있을 경우 예외처리된 행 추가
                    if(A_table_count > 1 or B_table_count > 1 or C_table_count > 1):
                        excel_data.append([file_name, "파일 내 추가 문건 존재", A_table_count, B_table_count, C_table_count] + [""] * 18)
